...
class ObjectNav_Agent(Agent):
    def __init__(self, args, follower=None) -> None:
        ...
        # ------------------------------------------------------------------
        self.classes=["potted plant", "table", "chair", "couch", "bed", "toilet", "tv", "stairs", "balustrade", "rail", "stool", "camera","stairs"]
        ...
        self.red_sem_pred = load_rednet_model(model_path="./rednet_semmap_mp3d_40.pth", device=torch.device(self.device))
    ...
    def reset(self, env) -> None:
        ...
        self.init_map_and_pose()
        self.init_map_and_navigation_param()
        self.stairs_ids = detect_stairs_ids(env)
        # self.stairs_pcd = o3d.geometry.PointCloud()
        self.stairs_pcds_dict = {}
        
        self.vis_stairs_id = []
        self.goal_id = -1
        # self.target_history = []
        print(f"楼梯代号：{self.stairs_ids}")

        self.position_history = deque(maxlen=50)  # maxlen=10 表示最多保存10个元素
        self.goal_history = deque(maxlen=50)
        # self.goal_map_history = deque(maxlen=35)
        # self.stuck_threshold = 10  # 连续10个相同目标点判定为卡住
        self.all_same = False
        self.yaun_goal = False
        self.unreachable_points = set()

        self.upstair_flag = False  # 新增标志位需要重置
        self.downstair_flag = False  # 新增标志位需要重置
        self.another_floor = False  # 新增标志位需要重置
        self.stairs_init_goal = False  # 新增标志位需要重置
        self.downstairs_flag = False  # 新增标志位需要重置
        self.upstairs_flag = False  # 新增标志位需要重置
        self.eve_angle = 0
        self.forward_staris = 0
        self.backward_staris = 0
    ...
    def init_map_and_navigation_param(self):
        ...
        self.stairs_init_goal = False
        self.downstairs_flag = False
        self.upstairs_flag = False
    ...
    def act(self, env, observations: Observations, agent_state, send_queue= Queue(), receive_queue= Queue()):
        ...
        if self.l_step == 0:
            ...
            # if cate_object == 'chair':
            #     cate_object = 'couch'
            if cate_object == 'sofa':
                cate_object = 'couch'
            if cate_object == 'tv_monitor':
                cate_object = 'tv'
            if cate_object == 'plant':
                cate_object = 'potted plant'
            
            if cate_object == 'tv' or cate_object == 'potted plant' or cate_object == 'chair' or cate_object == 'toilet' :
        ...
        current_position = (camera_position[0], camera_position[2])
        self.position_history.append(current_position)
        # self.goal_history.append()
        if len(self.position_history) == self.position_history.maxlen:
        # 检查所有历史目标点是否相同
            first_target = self.position_history[0]
            self.all_same = all(target == first_target for target in self.position_history)
            positions_array = np.array(self.position_history)
            distances = np.sqrt(np.sum((positions_array[1:] - first_target)**2, axis=1))
            self.all_same = np.all(distances < 0.8)
        ...
        stairs_pcds = detect_stairs(self.stairs_ids,image_rgb,depth,observations['semantic'],self.camera_K, camera_matrix_T)
        if stairs_pcds is not None:
            all_empty = all(pcd.is_empty() for pcd in stairs_pcds.values())
            if not all_empty:
                # 初始化存储每个楼梯点云的字典（如果还没有）
                if not hasattr(self, 'stairs_pcds_dict'):
                    self.stairs_pcds_dict = {}
                
                # 遍历每个楼梯ID的点云
                for stairs_id, pcd in stairs_pcds.items():
                    if not pcd.is_empty():
                        points = np.asarray(pcd.points)
                        print(f"楼梯ID {stairs_id} 的点云点数: {len(points)}")
                        
                        # 存储或更新该ID的点云
                        if stairs_id not in self.stairs_pcds_dict:
                            # 创建新的点云对象
                            new_pcd = o3d.geometry.PointCloud()
                            new_pcd.points = o3d.utility.Vector3dVector(points)
                            self.stairs_pcds_dict[stairs_id] = new_pcd
                        else:
                            # 合并到已有的点云
                            self.stairs_pcds_dict[stairs_id].points.extend(pcd.points)
                            # 对单个楼梯的点云进行下采样
                            self.stairs_pcds_dict[stairs_id] = self.stairs_pcds_dict[stairs_id].voxel_down_sample(voxel_size=0.02)
        

            # if np.max(points[:, 1]) > camera_position[1] - 0.8:
            #     # 有高点
            #     max_point = points[np.argmax(points[:, 1])]
            #     upstair_candidate_objects.append(max_point)
            # else:
            #     min_point = points[np.argmin(points[:, 1])]
            #     downstair_candidate_objects.append(min_point)
        ...
        obs_i_values, obs_j_values = self.update_map(self.point_sum, camera_position, self.args.map_height_cm / 100.0 /2.0)

        if self.another_floor:
            self.update_stairs_obstacle_map(self.stairs_pcds_dict)
        elif self.all_same:
            pass
        else:
            self.zengjia_stairs_obstacle_map(self.stairs_pcds_dict)

        if self.all_same:
            # input()
            print('相同')
            o_f_pos = np.argwhere(self.goal_map == 1)
            o_stg = o_f_pos[0]
            
            # 获取坐标
            x, y = o_stg[0], o_stg[1]

            self.unreachable_points.add((x, y))
            
            # stg, stop, plan_path = self._get_stg(self.obstacle_map, self.current_grid_pose, np.copy(self.goal_map))
            ...
            # path_np = np.array(plan_path).astype(int)

            # self.obstacle_map[path_np[:, 0] ,path_np[:, 1] ] = 1
        ...
        if len(target_point_list) > 0 :
            target_point_list = [p for p in target_point_list 
                        if (int(p[0]), int(p[1])) not in self.unreachable_points]

            print(f"过滤后剩余 {len(target_point_list)} 个点")

        if self.all_same and len(target_point_list) >=2:
            self.yaun_goal = True
        ...
        if len(self.objects) > 0:
            ...
            print("\n=== 候选物体列表 ===")
            cate_object = category_to_id[observations['objectgoal'][0]]
            print(f"搜索物体{cate_object}")
            # # for i, obj in enumerate(self.objects):
            # for i in range(len(self.objects)):
            #     # obj_index = self.objects.index(obj)
            #     # class_name = self.objects[i]['class_name']
            #     # conf = self.objects[i]['conf']
            #     # detections = self.objects[i]["num_detections"]
            #     pr_class_name = self.objects[i]['class_name'][0]
            #     pr_conf = max(self.objects[i]['conf'])
            #     pr_detections = self.objects[i]["num_detections"]
            #     print(f"{i+1}. 物体: {pr_class_name}, 检测次数: {pr_detections}, 相似度:{similarities[i]},置信度: {pr_conf}")
            ...
            if "couch" in self.text_queries:
                ...
                candidate_objects = [self.objects[i] for i in range(len(self.objects)) 
                                if (((similarities[i] > similarity_threshold +0.01 and \
                                    self.objects[i]['num_detections'] > 2) or (self.objects[i]['num_detections'] > 10 and max(self.objects[i]['conf']) > 0.80) ) \
                                    or \
                ...
                # for i in range(len(self.objects)):
                #     print(self.objects[i]['class_name'][np.argmax(self.objects[i]['conf'])])
                #     print(self.text_queries)
        ...
        if self.step_num >= 750:
            print(f'self.step_num:{self.step_num}')
        if not self.found_goal and (category_to_id[observations['objectgoal'][0]] == 'bed' or  category_to_id[observations['objectgoal'][0]] == 'plant' 
        or (category_to_id[observations['objectgoal'][0]] == 'toilet' and len(self.stairs_pcds_dict) >= 2)) :
            if hasattr(self, 'stairs_pcds_dict'):
                for stairs_id, stairs_pcd in self.stairs_pcds_dict.items():
                    points_np = np.asarray(stairs_pcd.points)
                    highest_point_idx = np.argmax(points_np[:, 1])  # 1是y轴的索引
                    highest_point = stairs_pcd.points[highest_point_idx]

                    # 计算整个点云的均值
                    mean_point = np.mean(points_np, axis=0)
                    
                    # if mean_point[1] < camera_position[1] - 0.8 :
                    #     self.downstairs_id = stairs_id
                    # if mean_point[1] > camera_position[1] - 0.1 :
                    #     self.upstairs_id = stairs_id

                    if highest_point[1] > camera_position[1]-0.1:
                            print(f'{category_to_id[observations["objectgoal"][0]]}优先上楼！！！')
                            self.upstairs_flag = True
                            self.goal_id = stairs_id
                            # self.vis_stairs_id.append(self.goal_id)
                            self.another_floor = True
                    if stairs_id not in self.vis_stairs_id:
                        ...
                        if highest_point[1] > camera_position[1]-0.1:
                            ...
                            # self.vis_stairs_id.append(self.goal_id)
                            self.another_floor = True
                        elif mean_point[1] < camera_position[1]-0.8 and  self.goal_id != stairs_id:
                            print(stairs_id)
                            # input()
                            self.vis_stairs_id.append(stairs_id)
                            

        x, y, z = camera_position[0], camera_position[1], camera_position[2]  # 默认值
        if not self.found_goal and not self.another_floor :
            stg = None
            ...
            if len(clip_candidate_objects) > 0 and \
                ...
                # print(f'len(target_point_list):{len(target_point_list)}')
                self.no_frontiers_count = 0
                simi_max_score = []
                stair_score = []
                ...
                print(f'没有边界了')
                if self.stairs_pcds_dict is not None:
                    all_empty = all(pcd.is_empty() for pcd in self.stairs_pcds_dict.values())
                    if not all_empty:
                        self.another_floor = True
                        for stairs_id, stairs_pcd in self.stairs_pcds_dict.items():
                            if stairs_id not in self.vis_stairs_id:
                            # if not hasattr(self, 'goal_id') or self.goal_id is None:
                                self.goal_id = stairs_id
                                
                                # points_np = np.asarray(stairs_pcd.points)
                                # highest_point_idx = np.argmax(points_np[:, 1])  # 1是y轴的索引
                                # highest_point = stairs_pcd.points[highest_point_idx]
                                # print(f'{highest_point}优先上楼！！！')
                                # print(f'highest_point：{highest_point[1]}')
                                # print(f'camera_position[1]:{camera_position[1]}')
                                # # 计算整个点云的均值
                                # mean_point = np.mean(points_np, axis=0)
                                
                                # print(f"楼梯 {stairs_id} 的点云均值: {mean_point}")
                                # print(f"  均值坐标: x={mean_point[0]:.3f}, y={mean_point[1]:.3f}, z={mean_point[2]:.3f}")
                                # if highest_point[1] > camera_position[1]-0.1:
                                #     self.upstairs_flag = True
                                break
                        # self.goal_id = next(iter(self.stairs_pcds_dict))
                ...
                #     if max(similarities) > 0.25 and ("potted plant" in self.text_queries or "chair" in self.text_queries or "tv" in self.text_queries):
                #         self.found_goal = True
                #     else:
                #         self.another_floor = True
        ...
        if  not self.found_goal and self.another_floor:

            # input('‘楼梯中’')
            
            # print('上下楼去。！！！')
            # if len(self.stairs_pcd.points) > 0:
            jinru_stair_flag = False
            for stairs_id, pcd in self.stairs_pcds_dict.items():
                
                if stairs_id == self.goal_id  and (self.goal_id not in self.vis_stairs_id ):#or category_to_id[observations['objectgoal'][0]] == 'bed'

                    # print(f'self.goal_id:{self.goal_id}')
                    jinru_stair_flag = True

                    points_np = np.asarray(pcd.points)
                    highest_point_idx = np.argmax(points_np[:, 1])  # 1是y轴的索引
                    highest_point = pcd.points[highest_point_idx]
                    lowest_point_idx = np.argmin(points_np[:, 1])  # 1是y轴的索引
                    lowest_point = pcd.points[lowest_point_idx]

                    if  self.upstairs_flag:
                        print('上楼！！！')
                        if not self.stairs_init_goal:

                            x = lowest_point[0]
                            y = lowest_point[1] # 这是最高的y值
                            z = lowest_point[2]
                            x_distance = abs(x - camera_position[0])
                            y_distance = abs(y - camera_position[1])
                            z_distance = abs(z - camera_position[2])

                            # 计算欧几里得距离（直线距离）
                            euclidean_distance = np.sqrt(x_distance**2 + y_distance**2 + z_distance**2)
                            # print(f'最低点距离:{euclidean_distance}')
                            if euclidean_distance < 1.5:

                                self.stairs_init_goal = True
                        else:

                            x = highest_point[0]
                            y = highest_point[1] # 这是最高的y值
                            z = highest_point[2]

                            # if y < camera_position[1]:
                            #     y += 0.
                            ...
                            # print(f'goal_stairs_id:{self.goal_id}')
                            ...
                            if euclidean_distance < 1.2:
                                ...
                                self.stairs_init_goal = False
                                self.vis_stairs_id.append(self.goal_id)
                                self.forward_staris = 15
                                # input('上楼完成！！！')

                                # return 1
                    elif self.downstairs_flag:
                        ...
                        if highest_point[1] > camera_position[1]-0.1:
                            self.upstairs_flag = True
                        elif mean_point[1] < camera_position[1]-0.8:
                            self.downstairs_flag = True
            ...
            if jinru_stair_flag:
                ...
                stairs_i = x * 100 / self.args.map_resolution + int(self.origins_grid[0])
                stairs_j = z * 100 / self.args.map_resolution + int(self.origins_grid[1])
                
                # 转换为整数索引stairs_pcds
                stairs_i_idx = int(stairs_i)
                stairs_j_idx = int(stairs_j)
                self.goal_map = np.zeros((self.local_w, self.local_h))
                self.goal_map[stairs_i_idx ,stairs_j_idx] = 1
        ...
        if len(plan_path) > 1 :
            ...
            self.plan_path_np = np.array(plan_path) 
            ...
            plan_path = np.array(plan_path) 
            self.plan_path_np = np.array(plan_path).astype(int)
            # print(f'len(plan_path_np):{len(self.plan_path_np)}')
            plan_path_x = (plan_path[:, 0] - int(self.origins_grid[0])) * self.args.map_resolution / 100.0
            plan_path_y = plan_path[:, 0] * 0
            plan_path_z = (plan_path[:, 1] - int(self.origins_grid[1])) * self.args.map_resolution / 100.0
        ...
        if self.forward_staris > 5:
            self.forward_staris -=1
            action = 1
            self.another_floor = False
            self.stairs_init_goal = False
            self.upstairs_flag = False

        elif self.forward_staris > 0:
            # input('右边！！！')
            self.forward_staris -=1
            action = 3
            self.another_floor = False
        ...
        if self.backward_staris > 5:
            self.backward_staris -=1
            action = 1
            self.another_floor = False
            self.stairs_init_goal = False
            self.downstairs_flag = False
        elif self.backward_staris > 0:
            self.backward_staris -=1
            action = 2
            self.another_floor = False
            self.stairs_init_goal = False
    ...
    def greedy_follower_act(self, plan_path):
        ...
        if self.another_floor and self.downstairs_flag:
        ...
        #         self.eve_angle += 30
            ...
            if self.eve_angle >= 30:
                action = 5
                self.eve_angle -= 30
            elif self.eve_angle <= -30:
                action = 4
                self.eve_angle += 30
        print(f'self.eve_angle:{self.eve_angle}')
    ...
    def _get_stg(self, grid, start, goal):
        ...
        if ("potted plant" in self.text_queries or "tv" in self.text_queries) and \
    ...
    # def update_stairs_obstacle_map(self,stairs_pcds_dict):
    #     for stairs_id, stairs_pcd in stairs_pcds_dict.items():
    ...
    #         if stairs_id == self.goal_id:
    #             self.obstacle_map[obs_i_values, obs_j_values] = 0
    #         else: 
    ...
    def update_stairs_obstacle_map(self, stairs_pcds_dict):
        ...
        for stairs_id, stairs_pcd in stairs_pcds_dict.items():
            ...
            # 创建一个临时的小地图来存放当前楼梯的点
            temp_map = np.zeros_like(self.obstacle_map)
            valid_idx = (0 <= obs_i_values) & (obs_i_values < self.obstacle_map.shape[0]) & \
                        (0 <= obs_j_values) & (obs_j_values < self.obstacle_map.shape[1])
            temp_map[obs_i_values[valid_idx], obs_j_values[valid_idx]] = 1
            
            # 对临时地图进行膨胀
            dilated_temp = cv2.dilate(temp_map.astype(np.uint8), kernel, iterations=1)
            
            if stairs_id == self.goal_id:
                # 目标楼梯区域设为0（可通行）
                self.obstacle_map[dilated_temp > 0] = 0
            else: 
                # 其他楼梯区域设为1（障碍物）
                self.obstacle_map[dilated_temp > 0] = 1
    
    def zengjia_stairs_obstacle_map(self, stairs_pcds_dict):
        # 定义膨胀核大小（可以根据需要调整）
        kernel_size = 5  # 膨胀半径
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))
    ...
    def update_map(self, point_sum, camera_position, height_diff):
        ...
        mask_explored = ( (points[:, 1] >= camera_position[1] - 1.2) & (points[:, 1] <= z_max)) & \
                    (points[:, 0] >= self.origins_real[0] - self.map_real_halfsize) & \
                    (points[:, 0] <= self.origins_real[0] + self.map_real_halfsize) & \
                    (points[:, 2] >= self.origins_real[1] - self.map_real_halfsize) & \
                    (points[:, 2] <= self.origins_real[1] + self.map_real_halfsize )
    ...
    def remove_full_points_cell(self, point_sum, camera_position):
        ...
        if len(points_filtered) > 0:
            # 找到y坐标最小的点（最低点）
            min_y_index = np.argmin(points_filtered[:, 1])
            lowest_point = points_filtered[min_y_index]
            
            print("\n=== 最低点信息 ===")
            print(f"最低点坐标: x={lowest_point[0]:.3f}, y={lowest_point[1]:.3f}, z={lowest_point[2]:.3f}")
            print(f"相机位置y: {camera_position[1]:.3f}")
            print(f"高度差: {camera_position[1] - lowest_point[1]:.3f} 米")
        colors_filtered = colors[mask]
